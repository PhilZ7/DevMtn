...
var nameGreeting = nameCheck("something");
...
// var colorRating = faveColorFinder("pepper");
...
function makeCapital(string) {
  let newText = string;
  let capital = newText.toUpperCase();
  console.log(capital);
}
makeCapital("will it work?");
...
function chocolateBuy(goldBag) {
  return goldBag / 3;
}

totalFrogs = chocolateBuy(123);
console.log(`You can purchase ${totalFrogs} frogs`);
...
/*
  ...
  (give it the same name just add a 2 to the end of it) that fixes this bug. Store the returned value to a variable called `
            totalFrogs2 `.
*/

//CODE HERE

function chocolateBuy2(goldBag2) {
  chocFrogs = goldBag2 /3;
  chocFrogs = Math.floor(chocFrogs);
  return chocFrogs;
}

totalFrogs = chocolateBuy2(4);
console.log(`You can purchase ${chocFrogs} frogs`);
...
function takeNums(array) {
  let compareNums = array[0]
  for (let i = 1; i < array.length - 1; i++) {
    if(array[i] <= compareNums){
      return false;
    } else {
      compareNums = array[i];
    }
...
let arrayIsAscending = takeNums(sampleArray);
...
//let pondScope = [] d contain the variable names(as strings) accessible in the pond
//function.
